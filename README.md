...
| 量价 | 昨日量价配合（放量上涨/缩量上涨/放量下跌/缩量下跌） |
| OBV | 20日能量潮方向（上行/下行） |
| 多空量比 | 20日上涨日均量/下跌日均量（>1.2 偏多方 / <0.8 偏空方） |
| 中期量能 | 5日/60日均量比（判断中期放量/缩量） |
| 主力资金 | 今日主力净流入及占流通市值比、5日累计（仅A股，见下节） |
...
- **主力资金**（仅 A股指数/ETF/个股）：每张卡片价格下方一行 `主力 今日X · 5日Y`，
  红色=净流入、绿色=净流出（A股惯例）。数据来自东方财富"主力净流入"
  （超大单+大单口径），每轮运行刷新，历史逐日累积在 state 里保存 20 日。
- **板块热力图**：看板顶部按涨跌幅着色的行业板块热力图（数据=东财行业板块全量 ~490 个，
  含多级细分）。红=涨、绿=跌，颜色深浅=幅度；大图块另带主力净流入。
  点开"全部板块"看完整网格。悬停/长按图块显示领涨股。
- **已知限制**：东财历史资金流接口（push2his）在本机网络不可达，只能拿到当日快照，
  5日累计由程序逐日自积累——刚启用时显示"5日累计中(n/5)"，跑满 5 个交易日后自动完整。
